...
def zpl_to_image(zpl_code, dpi=8, width=4, height=6):
    ...
    try:
        ...
        st.error(
            f"Error from Labelary API: {response.status_code} - {response.text}"
        )
...
def describe_image(image, model, processor):
    ...
    try:
        ...
        if image.mode != "RGB":
            image = image.convert(
                "RGB")  # Convert image to RGB if not already in RGB
...
def optimize_zpl(zpl_code: str) -> str:
    ...
    return re.sub(r"(\^FO\d+,\d+)\s+(\^FD.*?)(\^FS)", r"\1 \2 \3",
                  optimized_code)
...
def fix_and_lint_zpl(zpl_code):
    ...
    fixed_commands = [
        cmd.strip() for cmd in commands if cmd.strip().startswith("^")
    ]
    fixed_code = "\n".join(fixed_commands)
...
def main():
    ...
    with st.sidebar:
        st.header("Settings")
        dpi = st.selectbox("DPI (dots per inch)",
                           options=[6, 8, 12, 24],
                           index=1)
        width = st.number_input("Label Width (inches)",
                                min_value=1.0,
                                max_value=24.0,
                                value=4.0,
                                step=0.1)
        height = st.number_input("Label Height (inches)",
                                 min_value=1.0,
                                 max_value=24.0,
                                 value=6.0,
                                 step=0.1)
    ...
    if st.button("Generate / Describe Image", key="generate_image_button"):
        ...
        if image:
            ...
            st.image(rotated_image,
                     caption="Generated Label Image",
                     use_container_width=True)

            if use_vit == "Yes" and model and processor:
                with st.spinner("Describing the image..."):
                    description = describe_image(rotated_image, model,
                                                 processor)
                st.subheader("Image Description")
                st.write(description)
        else:
            st.error(
                "Failed to generate the image. Please check your ZPL code.")
